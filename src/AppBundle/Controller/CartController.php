        ...
        $session = new Session();
            ...
            if (($request->request->get('load_cart')) && $request->request->get('load_cart') == 1) {
                ...
                if ($session->get("products") && count($session->get("products")) > 0) { //if we have session variable
                    ...
                    $chilernBook = ($repo->arrayKeyValueSearch($array, $key = "product_category", $value = 1));
                    $fictionBook = ($repo->arrayKeyValueSearch($array, $key = "product_category", $value = 2));
                    $discount_details = $repo->priceDiscount($chilernBook, $fictionBook);
